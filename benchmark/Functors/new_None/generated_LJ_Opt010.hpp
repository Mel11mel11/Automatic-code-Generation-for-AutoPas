...
class LJFunctor_Gen_Opt010_None : public Functor<Particle_T> {
public:
    explicit LJFunctor_Gen_Opt010_None(double sigma, double epsilon, bool newton3 = true, double cutoff = 0.0)
    ...
    void AoSFunctor(Particle_T& p1, Particle_T& p2) override {
        ...
        const double Fmag = -4*epsilon*(-12*fast_pow(inv_r, 13)*fast_pow(sigma, 12) + 6*fast_pow(inv_r, 7)*fast_pow(sigma, 6));

...
class KryptonFunctorGenerated_Gen_Opt110 : public Functor<Particle_T> {
public:
    explicit KryptonFunctorGenerated_Gen_Opt110(double A, double a1, double a2, double a_m1, double b, double C6, double C8, double C10, bool newton3 = true, double cutoff = 0.0)
    ...
    void AoSFunctor(Particle_T& p1, Particle_T& p2) override {
        ...
        const double x2 = std::pow(C10, 3);
        const double x3 = std::pow(C8, -3);
        const double x4 = b*x0;
        const double x5 = std::pow(C10, 6);
        const double x6 = std::pow(C6, 3);
        const double x7 = std::pow(C8, -8);
        const double x8 = std::pow(C10, 10);
        const double x9 = std::pow(C6, 6);
        const double x10 = std::pow(C8, -15);
        const double x11 = std::pow(r, 2);

        const double Fmag = -A*(a1 + 2*a2*r - a_m1/x11)*std::exp(a1*r + a2*x11 + a_m1/r) + C10*(b*x1 - tt9*x4)/std::pow(r, 10) - 10*C10*(1 - x1)/std::pow(r, 11) + C6*(b*tt6*x0 - tt5*x4)/std::pow(r, 6) - 6*C6*(-tt6*x0 + 1)/std::pow(r, 7) + C6*x2*x3*(b*tt12*x0 - tt11*x4)/std::pow(r, 12) - 12*C6*x2*x3*(-tt12*x0 + 1)/std::pow(r, 13) + C8*(b*tt8*x0 - tt7*x4)/std::pow(r, 8) - 8*C8*(-tt8*x0 + 1)/std::pow(r, 9) + x5*x6*x7*(b*tt14*x0 - tt13*x4)/std::pow(r, 14) - 14*x5*x6*x7*(-tt14*x0 + 1)/std::pow(r, 15) + x10*x8*x9*(b*tt16*x0 - tt15*x4)/std::pow(r, 16) - 16*x10*x8*x9*(-tt16*x0 + 1)/std::pow(r, 17);

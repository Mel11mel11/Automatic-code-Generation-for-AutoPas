...
public:
    ...
    void AoSFunctor(Particle_T& p1, Particle_T& p2) override {
        ...
        const double dx = r2[0]-r1[0]; // abs function?
        const double dy = r2[1]-r1[1];
        const double dz = r2[2]-r1[2];
        ...
        const std::array<double,3> F{ mag*dx*inv_r, mag*dy*inv_r, mag*dz*inv_r };
        p1.addF(F);

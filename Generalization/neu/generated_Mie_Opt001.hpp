...
class MieFunctor_Gen_Opt001 : public Functor<Particle_T> {
public:
    explicit MieFunctor_Gen_Opt001(double sigma, double epsilon, double n, double m, bool newton3 = true, double cutoff = 0.0)
    ...
    void AoSFunctor(Particle_T& p1, Particle_T& p2) override {
        ...
        const double Fmag = -C*epsilon*(inv_r*m*std::std::pow(inv_r*sigma, m) - inv_r*n*std::std::pow(inv_r*sigma, n));
